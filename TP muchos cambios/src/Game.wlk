...
object juego{
	method iniciar(){
		...
		//Poderes//
		game.addVisual(new ObjetoObtenerPoder(cantCombustibleDisminuido = 0)) // Quizá deberíamos poner el cantCombustdisminuido
																			// en grieta y mancha de combustible, así el auto y esto lo pueden usar
																			// sin problemas
...
//	const listaPosiciones= [ Acá van a ir posiciones que pueden salir ] Dejo esto comentado para que no moleste la alerta de wollok
...
class AutoEnemigo inherits Obstaculos{
	...
	method posicionInicial() = game.at(3,23) // Si se pone esto en la var position, no es necesario este method, para eso está el property
	method iniciar(){
		const velocidad = 0
		position = self.posicionInicial() // Si hacemos lo que dije arriba, se podría dejar solo la position, o sacar esto directamente?
		game.onTick(velocidad,"moverEnemigos",{self.mover()})
	}
	method mover(){
		position = position.down(1)
...
* 
* Preguntar como hacer que el mensaje de game.say se quite, se queda todo el tiempo aunque ya haya pasado la colision
